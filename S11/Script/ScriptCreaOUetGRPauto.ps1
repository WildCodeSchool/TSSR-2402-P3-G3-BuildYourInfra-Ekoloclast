            ...
            if ($service.Length -ge 3) {
                ...
                if (-not (Get-ADGroup -Filter { Name -eq $fonctionModif })) {
                    try {
                        ...
                        Write-Host "Le groupe $fonctionModif a été créé avec succes dans l'OU $grpPath" -ForegroundColor Green
                    }
                    catch {
                        Write-Host "Erreur lors de la creation du groupe $fonctionModif dans l'OU $grpPath : $_" -ForegroundColor Red
                    }
                }
                else {
            ...
            if ($fonctionModif -ne "") {
                ...
                if (-not (Get-ADGroup -Filter { Name -eq $fonctionModif })) {
                    try {
                        ...
                        Write-Host "Le groupe $fonctionModif a été créé avec succes dans l'OU $ouPath" -ForegroundColor Green
                    }
                    catch {
                        Write-Host "Erreur lors de la creation du groupe $fonctionModif dans l'OU $ouPath : $_" -ForegroundColor Red
                    }
                }
                else {
                    #Write-Host "Le groupe $fonctionModif existe déjà." -ForegroundColor Yellow
